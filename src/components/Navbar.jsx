...
function Navbar() {
  ...
  const getNavLinkClass = ({ isActive }) =>
    `px-3 py-2 rounded-md font-medium transition-colors duration-200 ${isActive ? "text-secondary" : "text-gray-300 hover:text-secondary"}`;
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out shadow-lg ${
        isScrolled || isNotHomePage ? "bg-gray-900/90 backdrop-blur-lg" : "bg-transparent"
      }`}
    >
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          {/* Logo */}
          <NavLink to="/" className="text-3xl font-bold text-secondary tracking-tight">
            Sentralokal
          </NavLink>
          ...
          <div className="md:hidden flex items-center">
            <button
              ...
              className="text-gray-300 hover:text-primary focus:outline-none transition-transform duration-300"
            >
